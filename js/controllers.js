...
cMovilControllers.controller('categoriesCtrl', ['$scope', 'localStorageService', 'Category', 'cMovilConfig', '$rootScope', 'User',
    function ($scope, localStorageService, Category, cMovilConfig, $rootScope, User) {
        ...
        // Prepare empty array
        $scope.categories_filtered = [];

        $scope.filterCategory = function () {
            if($scope.categories === undefined || $scope.categories === null)
                return;

            if($scope.categories.categoriasproductos === undefined || $scope.categories.categoriasproductos === null)
                return;

            let tempArray = $scope.categories.categoriasproductos;

            tempArray = tempArray.filter( item => item.estado == true && item.locale == localStorageService.get('lang'));

            tempArray.sort((a,b) => (a.orden < b.orden) ? 1 : -1); 

            $scope.categories_filtered = tempArray;
        }

        // get All categories
        Category.getAll(localStorageService.get('sucursal')).success(function (data) {
            $scope.categories = data;
            $scope.categories_filtered.length = 0;
        });
        ...
        $scope.$watch(
                function ($scope) {
                    ...
                    $scope.isMobile = window.screen.width < 767 ? true : false;
...
cMovilControllers.controller('productsByClientCtrl', ['cMovilConfig', '$filter', '$timeout', '$scope', '$routeParams', 'localStorageService', 'Product', 'WishList', 'Review',
    function (cMovilConfig, $filter, $timeout, $scope, $routeParams, localStorageService, Product, WishList, Review) {
        ...
        $scope.filterProduct = function () {
            ...
            let tempArray = $scope.products.productos;

            tempArray = tempArray.filter( item => item.activo == true && item.locale == localStorageService.get('lang'));

            if($scope.searchText !== "") tempArray = tempArray.filter( item => item.nombre.indexOf($scope.searchText) > -1);

            if($scope.reverse) tempArray.reverse();

            $scope.products_filtered = tempArray;
        }
        ...
        Product.getAll(localStorageService.get('sucursal'), $routeParams.categoria, $routeParams.tipo, withAlergenos).success(function (data) {
            ...
            $scope.products_filtered.length = 0;
        });
        ...
        Config.getAll($routeParams.sucursal).success(function (data) {
            if (data.success)
            {
                ...
                Language.getAll(data.Sucursal.id).success(function (data) {
                    ...
                    /* 
                    // add extra languages (Italian, Portuguese)
                    var update_idiomas = data.Idiomas;
                    console.log(update_idiomas);
                    ...
                    var extraLanguages = [
                        ...
                        },
                        {
                            ...
                            logo: 'pt.png',
                            nombre: 'português',
                            nosotros: "<p>&nbsp; PORTUGUÊS</p>"
                        }
                    ];
                    extraLanguages.forEach((langItem) => {
                        update_idiomas.push(langItem);
                    })

                    console.log(update_idiomas);

                    localStorageService.set('languages', update_idiomas);
                    */
                });
...
/* Controller User */
cMovilControllers.controller('userCtrl', ['$rootScope', '$scope', 'User', '$location', 'localStorageService', '$window', 'cMovilConfig',
    function ($rootScope, $scope, User, $location, localStorageService, $window, cMovilConfig) {
